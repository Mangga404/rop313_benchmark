#!/usr/bin/env python3
"""Calculate OROP reduction potentials from completed solution-phase Gibbs energies.

For every ID retained in ``orop_reduction_potential.csv`` this script applies

    E_red = ((G_sol(N) - G_sol(red)) / F) - reference_shift

for a one-electron reduction.  Gaussian Gibbs energies are in Hartree, so the
energy difference is converted to kcal mol-1 before division by the Faraday
constant in kcal mol-1 V-1.  The reference shift is always read directly from
the matching OROP ``.shift`` file in the original ROP313 ZIP archive.
"""
...
# CODATA/NIST-compatible conversion constants.  Their ratio is the
# Hartree-to-volt conversion for a one-electron electrochemical process.
...
def read_experimental_potentials(csv_path: Path) -> dict[int, float]:
    if not csv_path.is_file():
        raise WorkflowError(f"Reduction-potential CSV not found: {csv_path}")
    ...
    if not values:
        raise WorkflowError(f"{csv_path.name} contains no reduction-potential records.")
    return values
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Calculate OROP reduction potentials from N/reduced solution Gibbs energies."
    )
    parser.add_argument("--neutral-energy-csv", type=Path, default=root / "orop_neutral_single_point_energy.csv")
    parser.add_argument("--reduced-energy-csv", type=Path, default=root / "rop313_energy_red.csv")
    parser.add_argument("--experimental-csv", type=Path, default=root / "orop_reduction_potential.csv")
    parser.add_argument("--zip", type=Path, default=root / "data" / "jp0c05052_si_002.zip")
    parser.add_argument(
        "--comparison-csv", type=Path, default=root / "orop_reduction_potential_comparison.csv"
    )
    parser.add_argument(
        "--statistics-csv", type=Path, default=root / "orop_reduction_potential_statistics.csv"
    )
    args = parser.parse_args()

    neutral = read_gibbs_energies(args.neutral_energy_csv.resolve(), "Neutral-state")
    reduced = read_gibbs_energies(args.reduced_energy_csv.resolve(), "Reduced-state")
    ...
    missing_reduced = [identifier for identifier in requested_ids if identifier not in reduced]
    completed_ids = [
        identifier
        for identifier in requested_ids
        if identifier not in missing_neutral and identifier not in missing_reduced
    ]
    if not completed_ids:
        raise WorkflowError("No reduction-potential IDs have both neutral and reduced Gibbs energies.")

    shifts = read_reference_shifts(args.zip.resolve(), completed_ids)
    rows: list[dict[str, str]] = []
    errors: list[float] = []
    for identifier in completed_ids:
        # Reduction potential is -DeltaG_red/F.  Defining the difference as
        # G(N)-G(red) therefore gives the positive form used below.
        delta_g_hartree = neutral[identifier] - reduced[identifier]
        ...
        rows.append({
            "id": str(identifier),
            "experimental_reduction_potential_v": f"{experimental[identifier]:.8f}",
            "reference_shift_v": f"{shifts[identifier]:.8f}",
            "g_sol_neutral_hartree": f"{neutral[identifier]:.12f}",
            "g_sol_reduced_hartree": f"{reduced[identifier]:.12f}",
            "delta_g_neutral_minus_reduced_hartree": f"{delta_g_hartree:.12f}",
            "delta_g_kcal_mol": f"{delta_g_kcal_mol:.8f}",
            "unshifted_potential_v": f"{unshifted_potential:.8f}",
            "calculated_reduction_potential_v": f"{calculated_potential:.8f}",
    ...
    write_csv(
        comparison_csv,
        [
            "id", "experimental_reduction_potential_v", "reference_shift_v",
            "g_sol_neutral_hartree", "g_sol_reduced_hartree",
            "delta_g_neutral_minus_reduced_hartree", "delta_g_kcal_mol",
            "unshifted_potential_v", "calculated_reduction_potential_v",
            "error_calculated_minus_experimental_v", "absolute_error_v", "squared_error_v2",
        ],
        rows,
    )
    statistic_rows = [
        {"metric": "formula", "value": "((G_sol(N)-G_sol(red))/F)-reference_shift"},
        ...
        {"metric": "experimental_reduction_ids", "value": str(len(requested_ids))},
        ...
        {"metric": "missing_reduced_energy_ids", "value": ",".join(map(str, missing_reduced)) or "none"},
